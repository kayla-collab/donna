...
- **Secure AI integration** using Cloudflare Workers AI (Llama 3.1) and a backend worker (`worker/clarity-worker.js`) that keeps logic server-side and supports image processing capabilities.
...
## Secure AI Setup (Cloudflare Workers AI)
1. **Configuration**:
   - The project uses `wrangler.jsonc` to configure the `AI` binding.
   - The backend uses `@cf/meta/llama-3.1-8b-instruct` model.
   - Image processing endpoint is available at `/process-image`.

2. **Prepare environment secrets** (never commit sensitive data):
   ```bash
...
4. **Point the frontend at the proxy endpoint:**
...
1. Create a `image-function` worker on Cloudflare to handle the image processing logic.
2. Re-enable the `image-function` service binding in `wrangler.jsonc` once the worker exists.
3. Replace placeholder media with hosted video content or interactive demos once available.
4. Evaluate performance optimizations (image compression, preloading critical fonts) before launch.
...
- Chat responses rely on Cloudflare Workers AI.
